"""MCP tool handler implementations for Scion.
...
# ---------------------------------------------------------------------------
_scion_instance = None
_scion_lock = asyncio.Lock()
...
# Lifecycle helpers
# ---------------------------------------------------------------------------
async def _get_scion():
    """Lazy-initialise the Scion engine."""
    global _scion_instance
    if _scion_instance is not None and _scion_instance.is_initialized():
        return _scion_instance

    async with _scion_lock:
        if _scion_instance is not None and _scion_instance.is_initialized():
            return _scion_instance

        logger.info("Initializing Scion engine ...")
        from scion.host_detection import build_grounding_config_path, build_llm_kwargs
        from scion.tool_layer import Scion, ScionConfig

        env_model = os.environ.get("SCION_MODEL", "")
        workspace = os.environ.get("SCION_WORKSPACE")
        max_iter = int(os.environ.get("SCION_MAX_ITERATIONS", "20"))
        enable_rec = os.environ.get("SCION_ENABLE_RECORDING", "true").lower() in ("true", "1", "yes")

        backend_scope_raw = os.environ.get("SCION_BACKEND_SCOPE")
        ...
        _scion_instance = Scion(config=config)
        await _scion_instance.initialize()
        logger.info("Scion engine ready (model=%s).", model)

        # Auto-register host bot skill directories from env
        host_skill_dirs_raw = os.environ.get("SCION_HOST_SKILL_DIRS", "")
        if host_skill_dirs_raw:
            dirs = [d.strip() for d in host_skill_dirs_raw.split(",") if d.strip()]
            if dirs:
                await _auto_register_skill_dirs(dirs)
                logger.info("Auto-registered host skill dirs from SCION_HOST_SKILL_DIRS: %s", dirs)

        return _scion_instance


def _get_store():
    """Get SkillStore — reuses Scion's internal instance when available."""
    global _standalone_store
    if _scion_instance and _scion_instance.is_initialized():
        internal = getattr(_scion_instance, "_skill_store", None)
...
def _is_auto_import_enabled() -> bool:
    ...
    """
    if _scion_instance and _scion_instance.is_initialized():
        gc = getattr(_scion_instance, "_grounding_config", None)
        if gc and gc.skills:
...
def _get_cloud_client():
    """Get a ScionClient instance (raises CloudError if not configured)."""
    from scion.cloud.auth import get_scion_auth
    from scion.cloud.client import ScionClient

    auth_headers, api_base = get_scion_auth()
    return ScionClient(auth_headers, api_base)
...
def _write_upload_meta(skill_dir: Path, info: Dict[str, Any]) -> None:
    ...
    meta = {
        ...
        "created_by": info.get("created_by", "scion"),
        "tags": info.get("tags", []),
    }
...
async def _auto_register_skill_dirs(skill_dirs: List[str]) -> int:
    """Register bot skill directories into Scion's SkillRegistry + DB."""
    ...
    scion = await _get_scion()
    registry = scion._skill_registry
    if not registry:
...
async def _cloud_search_and_import(task: str, limit: int = 8) -> List[Dict[str, Any]]:
    ...
    if not _is_auto_import_enabled():
        ...
        return []
    try:
        from scion.cloud.embedding import generate_embedding, resolve_embedding_api
        from scion.cloud.search import (
            SkillSearchEngine,
...
async def _do_import_cloud_skill(skill_id: str, target_dir: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target_dir:
        ...
        if host_ws:
            ...
            scion = await _get_scion()
            skill_cfg = scion._grounding_config.skills if scion._grounding_config else None
...
def _format_task_result(result: Dict[str, Any]) -> Dict[str, Any]:
    """Format an Scion execution result for MCP transport."""
    ...
    if raw_evolved:
        ...
        if names:
            output["action_required"] = (
                f"Scion auto-evolved {len(names)} skill(s): {', '.join(names)}. "
...
def _json_error(error_msg: str, *, error_code: str = "VALIDATION_ERROR") -> str:
    ...
    from scion.errors import safe_error_response
...
async def execute_task(
    ...
    """Execute a task with Scion's full grounding engine.
    ...
    Args:
        ...
        workspace_dir: Working directory. Defaults to SCION_WORKSPACE env.
        max_iterations: Max agent iterations (default: 20).
        skill_dirs: Bot's skill directories to auto-register so Scion
...
async def search_skills(
    ...
    **Scope difference from execute_task**:
    ...
    """
    try:
        from scion.cloud.search import hybrid_search_skills
...
async def fix_skill(
    ...
    The skill does NOT need to be pre-registered in Scion —
    provide the skill directory path and Scion will register it
    ...
    """
    try:
        from scion.skill_engine.evolution.models import EvolutionContext, EvolutionTrigger
        ...
        _write_upload_meta(
            new_skill_dir,
            {
                ...
                "created_by": new_record.lineage.created_by or "scion",
...
async def health_check() -> str:
    ...
    """
    from scion.observability.health import health
...
async def get_metrics() -> str:
    ...
    Returns all Scion metrics in Prometheus exposition format, suitable
...
def register_handlers(mcp) -> None:
    ...
    Called by ``create_mcp_app()`` in ``scion.mcp.server``.
    """
