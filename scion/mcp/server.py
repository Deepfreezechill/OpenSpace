"""Scion MCP Server — bootstrap, FastMCP creation, and entry point.

Extracted from ``mcp_server.py`` (Epic 4.9) to consolidate all MCP server
concerns under ``scion/mcp/``.
...
def create_mcp_app():
    ...
    try:
        if "description" in inspect.signature(FastMCP.__init__).parameters:
            kwargs["description"] = "Scion: Unite the Agents. Evolve the Mind. Rebuild the World."
    except (TypeError, ValueError):
        pass

    app = FastMCP("Scion", **kwargs)
...
def _register_health_probes() -> None:
    ...
    from scion.observability.health import HealthProbe, health
...
_EPILOG = """\
examples:
  scion-mcp                          # stdio transport (local, no auth)
  scion-mcp --transport streamable-http  # HTTP (requires bearer token)
  scion-mcp --transport sse --port 9000  # SSE on custom port

environment variables:
  SCION_MCP_HOST             Bind address (default: 0.0.0.0)
  SCION_MCP_PORT             Port number (default: 8000)
  SCION_MCP_TRANSPORT        Transport: stdio|sse|streamable-http
  SCION_MCP_BEARER_TOKEN     Auth token (REQUIRED for HTTP transports)
  SCION_LOG_LEVEL            Logging: DEBUG|INFO|WARNING|ERROR
  SCION_SHUTDOWN_TIMEOUT     Graceful shutdown seconds (default: 30)
  SCION_METRICS_ENABLED      Prometheus metrics: true|false

notes:
  HTTP transports (sse, streamable-http) require SCION_MCP_BEARER_TOKEN.
...
def _build_arg_parser(deploy_cfg=None):
    ...
    parser = argparse.ArgumentParser(
        prog="scion-mcp",
        description="Scion MCP Server — AI agent framework with tool execution",
        epilog=_EPILOG,
    ...
    parser.add_argument(
        ...
        "HTTP transports require SCION_MCP_BEARER_TOKEN.",
    )
    parser.add_argument(
        "--port",
        type=int,
...
def run_mcp_server(mcp=None) -> None:
    """Console-script entry point for ``scion-mcp``.
