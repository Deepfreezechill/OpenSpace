...
from scion.platforms import get_local_server_config
...
class GUIProvider(Provider):
    ...
    async def initialize(self) -> None:
        ...
        if not self.is_initialized:
            ...
            await self.create_session(
                SessionConfig(session_name=self.DEFAULT_SID, backend_type=BackendType.GUI, connection_params={})
            )
    ...
    async def create_session(self, session_config: SessionConfig) -> BaseSession:
        ...
        pkgs_prefix = get_config_value(
            conn_params, "pkgs_prefix", gui_config.pkgs_prefix.format(failsafe=failsafe_str, command="{command}")
        )
