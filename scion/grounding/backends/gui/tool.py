...
class GUIAgentTool(BaseTool):
    ...
    async def _arun(
        ...
        if not task_description:
            return ToolResult(status=ToolStatus.ERROR, error="task_description is required")
    ...
    async def _plan_next_action(
        ...
        if self.llm_client is None:
            ...
            return {"action_type": "FAIL", "reason": "LLM client not configured"}
        ...
        if is_anthropic:
            ...
            try:
                ...
                if commands == ["FAIL"]:
                    return {"action_type": "FAIL", "reason": "Anthropic planning failed"}

                if commands == ["DONE"]:
                    return {"action_type": "DONE", "reasoning": reasoning}
                ...
                if commands == ["SCREENSHOT"]:
                    ...
                    return {"action_type": "WAIT", "reasoning": reasoning or "Observing screen state"}
                ...
                if not commands:
                    return {"action_type": "FAIL", "reason": "No commands generated and no completion message"}

                # Return first command (Anthropic returns pyautogui commands directly)
                return {"action_type": "PYAUTOGUI_COMMAND", "command": commands[0], "reasoning": reasoning}

            except Exception as e:
                logger.error(f"Anthropic planning failed: {e}")
                return {"action_type": "FAIL", "reason": f"Planning error: {str(e)}"}
...
"""
    ...
    def _parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"action_type": "FAIL", "reason": "Invalid LLM response format"}

    async def _execute_planned_action(self, action: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def execute_action(self, action_type: str, parameters: Dict[str, Any]) -> ToolResult:
        """
        ...
        if result.get("status") == "success":
            return ToolResult(status=ToolStatus.SUCCESS, content=f"Executed {action_type}", metadata=result)
        else:
            return ToolResult(status=ToolStatus.ERROR, error=result.get("error", "Unknown error"), metadata=result)
    ...
    async def get_screenshot(self) -> ToolResult:
        ...
        if screenshot:
            return ToolResult(
                status=ToolStatus.SUCCESS, content=screenshot, metadata={"type": "screenshot", "size": len(screenshot)}
            )
        else:
            return ToolResult(status=ToolStatus.ERROR, error="Failed to capture screenshot")
    ...
    async def _record_intermediate_step(
        ...
        try:
            from scion.recording.manager import RecordingManager
