...
class MCPInstallerManager:
    ...
    async def _default_cli_prompt(self, message: str) -> bool:
        ...
        from scion.utils.display import colorize, print_separator
    ...
    async def _check_npm_package(self, package_name: str) -> bool:
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                "npm", "list", "-g", package_name, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
    ...
    async def _check_python_package(self, package_name: str) -> bool:
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                "uvx", clean_name, "--help", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
    ...
    async def _check_uv_pip_package(self, package_name: str) -> bool:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                "uv", "pip", "show", clean_name, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                "pip", "show", clean_name, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await process.communicate()
        ...
        try:
            ...
            if self._verbose or use_sudo:
                ...
                if use_sudo:
                    ...
                    process = await asyncio.create_subprocess_exec(
                        *install_command, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT
                    )
                ...
                while True:
                    try:
                        ...
                        print(
                            f"\r{Colors.BLUE}Installing dependencies...{Colors.RESET} {Colors.CYAN}{spinner[spinner_idx]}{Colors.RESET}",
                            end="",
                            flush=True,
                        )
            ...
            if process.returncode == 0:
                ...
                if is_permission_error and not use_sudo:
                    ...
                    print(
                        f"{Colors.RED}✗ Dependencies installation failed (return code: {process.returncode}){Colors.RESET}"
                    )
                    ...
                    print(
                        f"official documentation of the MCP server for manual installation instructions.{Colors.RESET}\n"
                    )
    ...
    async def ensure_dependencies(self, server_name: str, command: str, args: List[str]) -> bool:
    ...
    async def _ensure_dependencies_impl(self, server_name: str, command: str, args: List[str]) -> bool:
        ...
        if not self._check_command_available(command):
            error_msg = f"Command '{command}' is not available.\nPlease install the necessary tools first."
        ...
        if not install_cmd:
            ...
            print(
                f"{Colors.GRAY}Please refer to the MCP server's official documentation for installation instructions.{Colors.RESET}\n"
            )
