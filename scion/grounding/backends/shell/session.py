...
from scion.grounding.backends.shell.transport.connector import ShellConnector
from scion.grounding.backends.shell.transport.local_connector import LocalShellConnector
from scion.grounding.core.security.policies import SecurityPolicyManager
from scion.grounding.core.session import BaseSession
from scion.grounding.core.tool import BaseTool
from scion.grounding.core.types import BackendType, ToolResult, ToolStatus
...
class ShellSession(BaseSession):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(connector=connector, session_id=session_id, backend_type=BackendType.SHELL)
    ...
    async def initialize(self):
        ...
        if self.use_clawwork_productivity:
            from scion.grounding.backends.shell.productivity_tools import get_productivity_tools
            ...
            if extra:
                ...
                logger.warning(
                    "use_clawwork_productivity is True but livebench not available; productivity tools not added."
                )
...
If you already have the exact command/script to run, use run_shell instead."""
    ...
    def __init__(
        ...
        self._session = session
        # Use explicit model > SCION_MODEL env var > LLMClient default
        resolved_model = model or os.environ.get("SCION_MODEL") or None
        if resolved_model:
        ...
        self._default_env = default_env or {}
        self._default_conda_env = default_conda_env
        self._py_tool = PythonScriptTool(
            session,
            default_working_dir=default_working_dir,
            default_env=default_env,
            default_conda_env=default_conda_env,
        )
        self._bash_tool = BashScriptTool(
            session,
            default_working_dir=default_working_dir,
            default_env=default_env,
            default_conda_env=default_conda_env,
        ...
        if self._system_info is None:
            try:
                ...
                if base_url is not None:
                    try:
                        from scion.platforms import SystemInfoClient
                ...
                self._system_info = {"platform": platform, "username": username}
...
[TASK_FAILED: brief explanation of why it cannot be completed]"""
            ...
            try:
                ...
                if execution_result == "ERROR: No valid code block found":
                    ...
                    if no_code_counter >= 3:
                        final_message = (
                            f"Task failed after {iteration} steps: LLM failed to provide code blocks repeatedly"
                        )
                        return ToolResult(
                            status=ToolStatus.ERROR,
                ...
                conversation_history.extend(
                    [{"role": "assistant", "content": assistant_content}, {"role": "user", "content": feedback}]
                )
        ...
        return ToolResult(
            status=ToolStatus.ERROR, content=final_message, metadata={"tool": self._name, "code_history": code_history}
        )
        ...
        if "[TASK_COMPLETED:" in response:
            ...
            if has_error:
                ...
                return {
                    "completed": False,
                    "failed": False,
                    "reason": "Execution error occurred (LLM completion marker ignored)",
                }
