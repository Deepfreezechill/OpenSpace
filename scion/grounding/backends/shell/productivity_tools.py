...
these tools are added to the Shell backend so Scion agents have the same
...
def _dict_to_tool_result(out: Dict[str, Any]) -> ToolResult:
    """Convert ClawWork tool dict to Scion ToolResult."""
...
class CreateFileProductivityTool(_ProductivityToolBase):
    ...
    async def _arun(
        ...
        try:
            from scion.grounding.backends.shell.session import _parse_shell_result
