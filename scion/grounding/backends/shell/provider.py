...
from scion.platforms.config import get_local_server_config
...
class ShellProvider(Provider[ShellSession]):
    ...
    def _setup_security_policy(self, config: dict | None = None):
        ...
        logger.info(
            f"Shell security policy: allow_shell_commands={security_policy.allow_shell_commands}, "
    ...
    async def initialize(self) -> None:
        if not self.is_initialized:
            await self.create_session(
                SessionConfig(session_name=self.DEFAULT_SID, backend_type=BackendType.SHELL, connection_params={})
            )
