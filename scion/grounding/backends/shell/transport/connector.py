...
from scion.grounding.core.security import SecurityPolicyManager
from scion.grounding.core.transport.connectors import AioHttpConnector
...
class ShellConnector(AioHttpConnector):
    ...
    async def _retry_invoke(
        self, name: str, payload: Dict[str, Any], script_timeout: int, *, break_on_timeout: bool = False
    ):
        """
        ...
        for attempt in range(1, self.retry_times + 1):
            try:
                ...
                if break_on_timeout:
                    ...
                    raise RuntimeError(f"Script execution timed out after {script_timeout} seconds") from exc
                last_exc = exc
    ...
    async def run_python_script(
        ...
        *,
        ...
        return await self._retry_invoke("POST /run_python", payload, timeout, break_on_timeout=True)
    ...
    async def run_bash_script(
        ...
        *,
        ...
        result = await self._retry_invoke("POST /run_bash_script", payload, timeout, break_on_timeout=True)
