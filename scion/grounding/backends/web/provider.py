...
class WebProvider(Provider[WebSession]):
    ...
    async def initialize(self) -> None:
        ...
        if not self.is_initialized:
            ...
            await self.create_session(
                SessionConfig(session_name=self.DEFAULT_SID, backend_type=BackendType.WEB, connection_params={})
            )
    ...
    async def create_session(self, session_config: SessionConfig) -> WebSession:
        ...
        session = WebSession(session_id=session_name, config=session_config, auto_connect=True, auto_initialize=True)
