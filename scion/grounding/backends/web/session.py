...
from scion.grounding.core.transport.connectors import BaseConnector
...
# Load .env from scion package root (4 levels up), then CWD fallback.
_PKG_ENV = Path(__file__).resolve().parent.parent.parent.parent / ".env"  # scion/.env
if _PKG_ENV.is_file():
...
class WebSession(BaseSession):
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        if self.tools:
            ...
            return {"tools": [t.name for t in self.tools], "backend": BackendType.WEB.value}
    ...
    async def _arun(self, query: str) -> str:
        ...
        try:
            ...
            completion = await self._session.web_connector.client.chat.completions.create(
                model="perplexity/sonar-deep-research", messages=[{"role": "user", "content": query}]
            )
            ...
            logger.info("Begin to distill key points...")
