...
from scion.grounding.core.tool.base import BaseTool
...
class ToolRanker:
    ...
    def __init__(
        ...
        if model_name is None:
            try:
                from scion.config import get_config
        ...
        if cache_dir is None:
            cache_dir = PROJECT_ROOT / ".scion" / "embedding_cache"
        self._cache_dir = Path(cache_dir)

        # Log cache settings
        logger.info(
            f"ToolRanker initialized: enable_cache_persistence={enable_cache_persistence}, cache_dir={self._cache_dir}"
        )
    ...
    def _load_persistent_cache(self) -> None:
        ...
        try:
            ...
            if isinstance(data, dict) and data.get("version") == self.CACHE_VERSION:
                ...
                total = sum(len(tools) for backend in self._structured_cache.values() for tools in backend.values())
    ...
    def _keyword_search(self, query: str, tools: Iterable[BaseTool], top_k: int) -> List[Tuple[BaseTool, float]]:
        try:
    ...
    def _semantic_search(self, query: str, tools: Iterable[BaseTool], top_k: int) -> List[Tuple[BaseTool, float]]:
    ...
    def _hybrid_search(self, query: str, tools: Iterable[BaseTool], top_k: int) -> List[Tuple[BaseTool, float]]:
        # keyword filter
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        """
        stats = {"total_embeddings": 0, "backends": {}}
        ...
        for backend, servers in self._structured_cache.items():
            ...
            stats["backends"][backend] = {"total": backend_total, "servers": server_stats}
...
class SearchCoordinator(BaseTool):
    ...
    def __init__(
        ...
        *,
        ...
        enable_cache_persistence = resolve(
            enable_cache_persistence, "enable_cache_persistence", self.DEFAULT_CACHE_PERSISTENCE
        )
    ...
    async def _arun(
        ...
        *,
        ...
        should_use_llm_filter = self._llm and self._enable_llm_filter and mcp_count > self._llm_filter_threshold
                ...
                if total_count <= max_tools:
                    ...
                    if utility_tools:
                        utility_ranked = self._ranker.rank(
                            task_prompt, utility_tools, top_k=len(utility_tools), mode=SearchMode(mode)
                        )
                    ...
                    if domain_tools:
                        ...
                        all_domain_ranked = self._ranker.rank(
                            task_prompt, domain_tools, top_k=len(domain_tools), mode=SearchMode(mode)
                        )
        ...
        # Path 2: Plan-enhanced search (small MCP tool set)
            ...
            try:
                ranked = self._ranker.rank(enhanced_query, mcp_tools, top_k=max_tools, mode=SearchMode(mode))
                # Record all scores from semantic search
                self._record_tool_scores(debug_info, ranked, is_selected=True)
                mcp_result = [t for t, _ in ranked]
            except Exception:
                ranked = self._ranker._keyword_search(enhanced_query, mcp_tools, max_tools)
                self._record_tool_scores(debug_info, ranked, is_selected=True)
                mcp_result = [t for t, _ in ranked]
    ...
    def _record_tool_scores(
        self, debug_info: SearchDebugInfo, ranked: List[Tuple[BaseTool, float]], is_selected: bool = False
    ) -> None:
    ...
    def _populate_selected_tools(self, debug_info: SearchDebugInfo, tools: List[BaseTool]) -> None:
    ...
    async def _llm_filter_with_planning(
        self, task_prompt: str, tools: list[BaseTool]
...
- **domain_servers**: Server names that directly provide the main capabilities for the task.
  All tools from these servers will be considered. Be inclusive here."""
        ...
        messages_text = LLMClient.format_messages_to_text(
            [{"role": "system", "content": TOOL_FILTER_SYSTEM_PROMPT}, {"role": "user", "content": user_query}]
        )
