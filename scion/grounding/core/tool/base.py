...
class BaseTool(ABC):
    ...
    def __init__(
        self, schema: Optional[ToolSchema] = None, *, verbose: bool = False, handle_errors: bool = True
    ) -> None:
    ...
    def get_parameters_schema(cls) -> Dict[str, Any]:
        ...
        # If neither is overridden, raise error
            ...
            raise ValueError(f"{cls.__name__} must implement _run() or _arun() to define its parameters schema")
        ...
        PModel: type[BaseModel] = create_model(
            f"{cls.__name__}Params", __config__=ConfigDict(arbitrary_types_allowed=True), **fields
        )
    ...
    async def invoke(self, parameters: Dict[str, Any] | None = None, keep_session: bool = True, **kwargs) -> ToolResult:
        """
    ...
    async def _record_to_quality_manager(
        ...
        try:
            from scion.grounding.core.quality import get_quality_manager
