...
if TYPE_CHECKING:
    from scion.grounding.core.tool import BaseTool
...
class ToolQualityManager:
    ...
    async def record_llm_tool_issues(
        ...
        if updated:
            logger.info(
                f"Recorded {updated} LLM tool issue(s) into quality pipeline{f' (task={task_id})' if task_id else ''}"
            )
        return updated
    ...
    def get_llm_flagged_tools(
        ...
        """
        return [r for r in self._records.values() if r.llm_flagged_count >= min_flags]
...
```"""
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "avg_success_rate": (sum(r.success_rate for r in records) / len(records) if records else 0),
            "avg_quality_score": (sum(r.quality_score for r in records) / len(records) if records else 0),
            "tools_with_description_eval": sum(1 for r in records if r.description_quality),
            "tools_llm_flagged": sum(1 for r in records if r.llm_flagged_count > 0),
        }
    ...
    def get_top_tools(
        ...
        records = [
            ...
            if r.total_calls >= min_calls and (backend is None or r.backend == backend)
        ]
    ...
    def get_problematic_tools(
        ...
        return [
            r
            for r in self._records.values()
            if r.total_calls >= min_calls and r.recent_success_rate < success_rate_threshold
        ]
    ...
    def get_quality_report(self) -> Dict:
        ...
        for r in records:
            if r.backend not in backends:
                backends[r.backend] = {"tools": 0, "total_calls": 0, "success_count": 0, "servers": set()}
        ...
        for b in backends:
            ...
            backends[b]["success_rate"] = (
                backends[b]["success_count"] / backends[b]["total_calls"] if backends[b]["total_calls"] > 0 else 0
            )
        ...
        return {
            "summary": {
                ...
                "overall_success_rate": (
                    sum(r.success_count for r in records) / max(1, sum(r.total_calls for r in records))
                ),
                "avg_quality_score": (
                    sum(r.quality_score for r in tested_records) / len(tested_records) if tested_records else 0
                ),
            },
            "by_backend": backends,
            "top_tools": [
                {"key": r.tool_key, "score": r.quality_score, "success_rate": r.success_rate} for r in top_tools
            ],
            "problematic_tools": [
                {"key": r.tool_key, "success_rate": r.success_rate, "calls": r.total_calls} for r in problematic
            ],
    ...
    def _generate_recommendations(
        ...
        if problematic:
            ...
            recommendations.append(f"Review low-success tools: {', '.join(tool_names)}")
        ...
        if unevaluated:
            recommendations.append(f"{len(unevaluated)} tools need description quality evaluation")

        # Check for low description quality
        poor_docs = [r for r in records if r.description_quality and r.description_quality.overall_score < 0.5]
        if poor_docs:
            recommendations.append(f"{len(poor_docs)} tools have poor documentation quality")
    ...
    async def evolve(self, tools: List["BaseTool"]) -> Dict:
        ...
        problematic = self.get_problematic_tools()
        report["recommendations"] = self._generate_recommendations(list(self._records.values()), problematic)
    ...
    def get_tool_insights(self, tool: "BaseTool") -> Dict:
        ...
        recent_failures_count = sum(1 for e in record.recent_executions[-20:] if not e.success)
