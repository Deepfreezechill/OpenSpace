...
from scion.utils.cli_display import CLIDisplay
from scion.utils.display import colorize
from scion.utils.logging import Logger
from scion.utils.ui import ScionUI, create_ui
from scion.utils.ui_integration import UIIntegration

logger = Logger.get_logger(__name__)


class UIManager:
    def __init__(self, ui: Optional[ScionUI], ui_integration: Optional[UIIntegration]):
        self.ui = ui
    ...
    def _suppress_logs(self):
        log_names = ["scion", "scion.grounding", "scion.agents"]
...
async def _execute_task(scion: Scion, query: str, ui_manager: UIManager):
    await ui_manager.start_live_display()
    result = await scion.execute(query)
    await ui_manager.stop_live_display()
    ui_manager.print_summary(result)
    return result


async def interactive_mode(scion: Scion, ui_manager: UIManager):
    ...
    while True:
        try:
            ...
            await _execute_task(scion, query, ui_manager)
...
async def single_query_mode(scion: Scion, query: str, ui_manager: UIManager):
    CLIDisplay.print_task_header(query, title="▶ Single Query Execution")
    await _execute_task(scion, query, ui_manager)


def _print_status(scion: Scion):
    """Print system status"""
    from scion.utils.display import Box, BoxStyle

    box = Box(width=70, style=BoxStyle.ROUNDED, color="bl")
    print()
    print(box.text_line(colorize("System Status", "bl", bold=True), align="center", indent=4, text_color=""))
    print(box.separator_line(indent=4))

    status_lines = [
        f"Initialized: {colorize('Yes' if scion.is_initialized() else 'No', 'g' if scion.is_initialized() else 'rd')}",
        f"Running: {colorize('Yes' if scion.is_running() else 'No', 'y' if scion.is_running() else 'g')}",
        f"Model: {colorize(scion.config.llm_model, 'c')}",
    ]

    if scion.is_initialized():
        backends = scion.list_backends()
        status_lines.append(f"Backends: {colorize(', '.join(backends), 'c')}")

        sessions = scion.list_sessions()
...
def _create_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Scion - Self-Evolving Skill Worker & Community",
    ...
    cache_parser = subparsers.add_parser("refresh-cache", help="Refresh MCP tool cache (starts all servers once)")
    cache_parser.add_argument("--config", "-c", type=str, help="MCP configuration file path")
...
async def refresh_mcp_cache(config_path: Optional[str] = None):
    ...
    from scion.config import get_config, load_config
    from scion.grounding.backends.mcp import MCPProvider, get_tool_cache
    from scion.grounding.core.types import BackendType, SessionConfig
...
def _load_config(args) -> ScionConfig:
    """Load configuration"""
    cli_overrides = {}
    if args.model:
...
def _setup_ui(args) -> tuple[Optional[ScionUI], Optional[UIIntegration]]:
    if args.no_ui:
...
async def _initialize_scion(config: ScionConfig, args) -> Scion:
    scion = Scion(config)

    init_steps = [("Initializing Scion...", "loading")]
    CLIDisplay.print_initialization_progress(init_steps, show_header=False)

    if not args.config:
        original_log_level = Logger.get_logger("scion").level
        for log_name in ["scion", "scion.grounding", "scion.agents"]:
            Logger.get_logger(log_name).setLevel(logging.WARNING)

    await scion.initialize()
...
async def main():
    ...
    try:
        # Initialize Scion
        scion = await _initialize_scion(config, args)

        # Connect UI (if enabled)
        if ui_integration:
            ui_integration.attach_llm_client(scion._llm_client)
            ui_integration.attach_grounding_client(scion._grounding_client)
        ...
        if args.query:
            await single_query_mode(scion, args.query, ui_manager)
        else:
            await interactive_mode(scion, ui_manager)
