...
from scion.grounding.core.types import ToolResult, ToolSchema, ToolStatus
...
_PKG_ENV = Path(__file__).resolve().parent.parent / ".env"  # scion/.env
if _PKG_ENV.is_file():
...
Concise summary:"""
        ...
        response = await asyncio.wait_for(
            litellm.acompletion(model=model, messages=[{"role": "user", "content": prompt}], timeout=timeout),
            timeout=timeout + 5,
        )
    ...
    *,
    ...
    if original_len > summarize_threshold and enable_summarization:
        ...
        if summary:
            ...
            logger.warning(
                f"Tool result truncated for '{tool_name}': {original_len:,} -> {len(text_content):,} chars (summarization failed)"
            )
...
async def _execute_tool_call(
    ...
    if isinstance(arguments, dict) and tool.schema.parameters:
        ...
        for param in invalid_params:
            arguments.pop(param)
            logger.debug(f"Removed parameter '{param}' from {tool.schema.name} (not in tool schema)")

    return await tool.invoke(parameters=arguments, keep_session=True)
    ...
    async def _call_with_retry(self, **completion_kwargs):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                response = await asyncio.wait_for(litellm.acompletion(**completion_kwargs), timeout=self.timeout)
                return response
                ...
                is_rate_limit = any(
                    keyword in error_str for keyword in ["rate limit", "rate_limit", "too many requests", "429"]
                )
        ...
        **kwargs,
        ...
        if execute_tools and tool_calls and tools:
            ...
            for tool_call in tool_calls:
                ...
                if backend is None and tool_name:
                    ...
                    if backend is None:
                        self._logger.warning(
                            f"Could not resolve backend for tool '{tool_name}', recording will be skipped"
                        )
                ...
                if tool_name not in tool_map:
                    result = ToolResult(status=ToolStatus.ERROR, error=f"Tool '{tool_name}' not found")
                else:
                    try:
                        ...
                        if tool_result_callback and not result.is_error:
                            try:
                                result = await tool_result_callback(
                                    result=result, tool_name=tool_name, tool_call=tool_call, backend=backend
                                )
                            except Exception as e:
                                self._logger.warning(f"Tool result callback failed for {tool_name}: {e}")
                    except Exception as e:
                        result = ToolResult(status=ToolStatus.ERROR, error=str(e))
        ...
        if summary_prompt and tool_results:
            ...
            if summary_response.choices:
                ...
                current_messages.append({"role": "assistant", "content": iteration_summary})
