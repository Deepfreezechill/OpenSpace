"""
Scion Terminal UI System

Provides real-time CLI visualization for Scion execution flow.
...
class ScionUI:
    ...
    def print_summary(self, result: Dict[str, Any]):
        ...
        print(
            box.text_line(
                f"  Execution Time:  {colorize(f'{result.get("execution_time", 0):.2f}s', 'c')}",
                indent=4,
                text_color="",
            )
        )
        print(
            box.text_line(
                f"  Iterations:      {colorize(str(result.get('iterations', 0)), 'y')}", indent=4, text_color=""
            )
        )
        print(
            box.text_line(
                f"  Completed Tasks: {colorize(str(result.get('completed_tasks', 0)), 'g')}", indent=4, text_color=""
            )
        )
        ...
        if result.get("evaluation_results"):
            print(
                box.text_line(
                    f"  Evaluations:     {colorize(str(len(result['evaluation_results'])), 'bl')}",
                    indent=4,
                    text_color="",
                )
            )
...
def create_ui(enable_live: bool = True, compact: bool = False) -> ScionUI:
    """
    Create Scion UI instance

    Args:
        enable_live: Whether to enable live display updates
        compact: Use compact layout for smaller terminals
    """
    return ScionUI(enable_live=enable_live, compact=compact)
