...
def _load_log_level_from_config() -> int:
    """
    Load log_level from config_grounding.json and convert to SCION_DEBUG value.
    ...
    try:
        ...
        if config_path.exists():
            with open(config_path, "r", encoding="utf-8") as f:
                ...
                # Convert log level string to SCION_DEBUG value
...
SCION_DEBUG = _load_log_level_from_config()

# Default log directory and file pattern
# Use absolute path to scion/logs directory
DEFAULT_LOG_DIR = os.path.join(os.path.dirname(__file__), "..", "..", "logs")
DEFAULT_LOG_FILE_PATTERN = "scion_{timestamp}.log"
...
class Logger:
    ...
    3. Dynamically adapts log levels according to ``SCION_DEBUG``.
    """

    _ROOT_NAME = "scion"  # Package root name
    ...
    def _get_default_log_file() -> str:
        ...
        - logs/<script_name>/scion_2025-10-24_15-30-00.log
        """
        # Get the name of the main script
        script_name = "scion"  # Default name
        try:
            import __main__
    ...
    def get_logger(cls, name: Optional[str] = None) -> logging.Logger:
        """Return a logger with *name* (defaults to ``scion``).
    ...
    def configure(
        ...
        *,
        ...
        with cls._lock:
            ...
            # Select the logger to attach handlers to (root logger or scion).
    ...
    def _resolve_level(cls, level: Optional[int]) -> int:
        ...
        return {2: logging.DEBUG, 1: logging.INFO}.get(SCION_DEBUG, logging.WARNING)
...
# ``SCION_DEBUG`` (preferred) or legacy ``DEBUG`` environment variable.
_env_debug = os.getenv("SCION_DEBUG") or os.getenv("DEBUG")
...
# Get scion logger for internal logging
logger = Logger.get_logger()
logger.debug("Scion logging initialized")
