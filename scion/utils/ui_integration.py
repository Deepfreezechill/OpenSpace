"""
Scion UI Integration

Integrates the UI system with Scion core components.
...
from scion.utils.ui import AgentStatus, ScionUI
...
class UIIntegration:
    """
    UI Integration for Scion

    Connects Scion components with the UI system to provide real-time
    visualization of agent activities and execution flow.
    """

    def __init__(self, ui: ScionUI):
        """
    ...
    async def start_monitoring(self, poll_interval: float = 0.5):
        ...
        self._update_task = asyncio.create_task(self._monitor_loop(poll_interval))
    ...
    def on_llm_call(self, model: str, prompt_length: int):
        ...
        """
        self.ui.update_metrics(llm_calls=self.ui.metrics.get("llm_calls", 0) + 1)
...
def create_integration(ui: ScionUI) -> UIIntegration:
    """
