...
def _check_safety(text: str) -> list[str]:
    ...
    from scion.skill_engine.skill_utils import check_skill_safety

    return check_skill_safety(text)


def _is_safe(flags: list[str]) -> bool:
    from scion.skill_engine.skill_utils import is_skill_safe
...
class SkillSearchEngine:
    ...
    def _bm25_phase(
        ...
        from scion.skill_engine.skill_ranker import SkillCandidate, SkillRanker
    ...
    def _score_phase(
        ...
        from scion.cloud.embedding import cosine_similarity
...
def build_local_candidates(
    ...
    """
    from scion.cloud.embedding import build_skill_embedding_text
...
def build_cloud_candidates(
    ...
    Args:
        items: Items from ``ScionClient.fetch_metadata()``.
...
async def hybrid_search_skills(
    ...
    if source in ("all", "cloud"):
        try:
            ...
            if auth_headers:
                client = ScionClient(auth_headers, api_base)
                try:
                    from scion.cloud.embedding import resolve_embedding_api
