"""Scion cloud platform HTTP client.
...
class ScionClient:
    """HTTP client for the Scion cloud API.

    Args:
        auth_headers: Pre-resolved auth headers (from ``get_scion_auth``).
        api_base: API base URL (e.g. ``https://scion-skills.dev/api/v1``).
    """

    _DEFAULT_UA = "Scion-Client/1.0"

    def __init__(self, auth_headers: Dict[str, str], api_base: str):
        if not auth_headers:
            raise CloudError("No SCION_API_KEY configured. Register at https://scion-skills.dev to obtain a key.")
        self._headers = {
    ...
    def stage_artifact(self, skill_dir: Path) -> tuple[str, int]:
        ...
        boundary = f"----ScionUpload{os.urandom(8).hex()}"
    ...
    def upload_skill(
        ...
        *,
        ...
        """
        from scion.skill_engine.skill_utils import parse_frontmatter
