#!/usr/bin/env python3
"""Upload a skill to the Scion cloud platform.

Usage:
    scion-upload-skill --skill-dir ./my-skill --visibility public --origin imported
    scion-upload-skill --skill-dir ./my-skill --visibility private --origin fixed --parent-ids "parent_id"
"""
...
from scion.cloud.auth import get_api_base, get_auth_headers_or_exit
from scion.cloud.client import CloudError, ScionClient


def main() -> None:
    parser = argparse.ArgumentParser(
        prog="scion-upload-skill",
        description="Upload a skill to Scion's cloud community",
    )
    ...
    if args.dry_run:
        files = ScionClient._collect_files(skill_dir)
