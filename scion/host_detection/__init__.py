...
Supported host agents:
  ...
  - **nanobot** — ``~/.nanobot/config.json``  (``tools.mcpServers.scion.env``)
  - **openclaw** — ``~/.openclaw/openclaw.json``  (``skills.entries.scion.env``)
"""

import logging
from typing import Dict, Optional

from scion.host_detection.nanobot import (
...
from scion.host_detection.openclaw import (
...
from scion.host_detection.resolver import build_grounding_config_path, build_llm_kwargs
...
def read_host_mcp_env() -> Dict[str, str]:
    ...
    Resolution order:
      1. nanobot — ``tools.mcpServers.scion.env``
      2. openclaw — ``skills.entries.scion.env``
    ...
    # Try openclaw
    env = read_openclaw_skill_env("scion")
    if env:
...
def get_openai_api_key() -> Optional[str]:
    ...
    Resolution:
      ...
      3. openclaw config ``skills.entries.scion.env.OPENAI_API_KEY``
      4. None
    """
