...
Resolves the model name and litellm kwargs for Scion's LLM client,
...
logger = logging.getLogger("scion.host_detection")


def build_llm_kwargs(model: str) -> tuple[str, Dict[str, Any]]:
    """Build litellm kwargs and resolve model for Scion's LLM client.

    Resolution order (highest → lowest priority):

    Tier 1 — Explicit ``SCION_LLM_*`` env vars::

        SCION_LLM_API_KEY         → litellm ``api_key``
        SCION_LLM_API_BASE        → litellm ``api_base``
        SCION_LLM_EXTRA_HEADERS   → litellm ``extra_headers`` (JSON string)
    ...
    """
    from scion.host_detection.nanobot import try_read_nanobot_config
    ...
    api_key = os.environ.get("SCION_LLM_API_KEY")
    if api_key:
        kwargs["api_key"] = api_key
        source = "SCION_LLM_* env"

    api_base = os.environ.get("SCION_LLM_API_BASE")
    if api_base:
        kwargs["api_base"] = api_base

    extra_headers_raw = os.environ.get("SCION_LLM_EXTRA_HEADERS")
    if extra_headers_raw:
        try:
            ...
            logger.warning("Invalid JSON in SCION_LLM_EXTRA_HEADERS: %r", extra_headers_raw)

    llm_config_raw = os.environ.get("SCION_LLM_CONFIG")
    if llm_config_raw:
        try:
            llm_config = json.loads(llm_config_raw)
            if isinstance(llm_config, dict):
                kwargs.update(llm_config)
                source = "SCION_LLM_CONFIG env"
        except json.JSONDecodeError:
            logger.warning("Invalid JSON in SCION_LLM_CONFIG: %r", llm_config_raw)
...
def build_grounding_config_path() -> Optional[str]:
    ...
    Supports:
      * ``SCION_CONFIG_JSON``  — inline JSON string (written to a temp file)
      * ``SCION_CONFIG_PATH``  — path to a JSON config file

    Granular env-var overrides (``SCION_SHELL_*``, ``SCION_SKILLS_*``,
    etc.) are merged before writing.

    Returns:
        Path to the resolved config file, or None.
    """
    config_json_raw = os.environ.get("SCION_CONFIG_JSON", "").strip()
    overrides: Dict[str, Any] = {}
    if config_json_raw:
        try:
            overrides = json.loads(config_json_raw)
            if not isinstance(overrides, dict):
                logger.warning("SCION_CONFIG_JSON is not a dict, ignoring")
                overrides = {}
            else:
                logger.info("Loaded inline config from SCION_CONFIG_JSON")
        except json.JSONDecodeError as e:
            logger.warning("Invalid JSON in SCION_CONFIG_JSON: %s", e)

    # --- Granular env-var overrides ---
    conda_env = os.environ.get("SCION_SHELL_CONDA_ENV", "").strip()
    if conda_env:
        overrides.setdefault("shell", {})["conda_env"] = conda_env

    shell_wd = os.environ.get("SCION_SHELL_WORKING_DIR", "").strip()
    if shell_wd:
        overrides.setdefault("shell", {})["working_dir"] = shell_wd

    skills_dirs_raw = os.environ.get("SCION_SKILLS_DIRS", "").strip()
    if skills_dirs_raw:
        dirs = [d.strip() for d in skills_dirs_raw.split(",") if d.strip()]
        if dirs:
            overrides.setdefault("skills", {})["skill_dirs"] = dirs

    mcp_servers_raw = os.environ.get("SCION_MCP_SERVERS_JSON", "").strip()
    if mcp_servers_raw:
        try:
            ...
            logger.warning("Invalid JSON in SCION_MCP_SERVERS_JSON: %s", e)

    log_level = os.environ.get("SCION_LOG_LEVEL", "").strip().upper()
    if log_level and log_level in ("DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"):
        overrides["log_level"] = log_level

    if overrides:
        try:
            fd, tmp_path = tempfile.mkstemp(suffix=".json", prefix="scion_cfg_")
    ...
    return os.environ.get("SCION_CONFIG_PATH")
