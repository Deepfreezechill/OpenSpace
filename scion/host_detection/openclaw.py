...
Reads ``~/.openclaw/openclaw.json`` to auto-detect:
  ...
  - Skill-level env block (``skills.entries.scion.env``)
...
def get_openclaw_openai_api_key() -> Optional[str]:
    ...
    Checks ``skills.entries.scion.env.OPENAI_API_KEY`` first,
