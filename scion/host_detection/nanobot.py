...
Reads ``~/.nanobot/config.json`` to auto-detect:
  ...
  - MCP env block for the ``scion`` server
...
def read_nanobot_mcp_env() -> Dict[str, str]:
    """Read ``tools.mcpServers.scion.env`` from nanobot config.
    ...
    if not isinstance(mcp_servers, dict):
        return {}
    scion_cfg = mcp_servers.get("scion", {})
    if not isinstance(scion_cfg, dict):
        return {}
    env_block = scion_cfg.get("env", {})
