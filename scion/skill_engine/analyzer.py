...
Integration:
  Instantiated once during ``Scion.initialize()``.
  ``analyze_execution()`` is called in the ``finally`` block of ``Scion.execute()``.
"""
...
from scion.prompts import SkillEnginePrompts
...
if TYPE_CHECKING:
    from scion.grounding.core.quality import ToolQualityManager
...
class ExecutionAnalyzer:
    ...
    async def analyze_execution(
        ...
        Args:
            ...
            execution_result: The return value of ``Scion.execute()`` — contains status,
