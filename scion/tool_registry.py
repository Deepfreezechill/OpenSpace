...
Extracted from ``Scion`` (tool_layer.py) in Epic 4.1.  Owns:
...
class ToolRegistry:
    ...
    ----------
    config:
        The ``ScionConfig`` (for ``skill_registry_model``, ``llm_kwargs``).
    grounding_config:
    ...
    def discover(self) -> bool:
        ...
        Discovery order (earlier wins on name collision):
          1. ``SCION_HOST_SKILL_DIRS`` env — host agent skill directories
          2. ``config_grounding.json → skills.skill_dirs`` — user-specified
          3. ``scion/skills/``       — built-in skills (always present)
        ...
        host_dirs_raw = os.environ.get("SCION_HOST_SKILL_DIRS", "")
        if host_dirs_raw:
        ...
        if skill_cfg and skill_cfg.skill_dirs:
            for d in skill_cfg.skill_dirs:
                p = Path(d)
                if p in skill_paths:
                    continue  # Already added via SCION_HOST_SKILL_DIRS
                if p.exists():
                    skill_paths.append(p)
                else:
                    logger.warning(f"Configured skill dir does not exist: {d}")

        # 3. Built-in skills (scion/skills/)
