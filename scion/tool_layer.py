...
from scion.config.loader import get_agent_config
...
from scion.skill_engine import ExecutionAnalyzer, SkillRegistry, SkillStore
...
class ScionConfig:
    ...
    def __post_init__(self):
        ...
        logger.debug(f"ScionConfig initialized with model: {self.llm_model}")
...
class Scion:
    ...
    **Container-based** (Phase 1 seam — Phase 4 wires initialize)::
    ...
    ----------
    config:
        Application config.  Defaults to ``ScionConfig()``.
    container:
    ...
    def __init__(
        ...
        *,
        ...
        logger.debug("Scion instance created")
    ...
    def from_container(
        ...
        """Create an Scion instance backed by an AppContainer."""
    ...
    async def initialize(self) -> None:
        if self._initialized:
            logger.warning("Scion already initialized")
            return

        logger.info("Initializing Scion...")
        ...
        try:
            ...
            logger.info("=" * 60)
            logger.info("Scion ready to use!")
            logger.info("=" * 60)
    ...
    def _load_grounding_config(self):
        ...
        if self.config.grounding_config_path:
            from scion.config.constants import CONFIG_GROUNDING, CONFIG_SECURITY
            from scion.config.loader import CONFIG_DIR
        ...
        _default_max_iter = ScionConfig().grounding_max_iterations
    ...
    async def execute(
        ...
        """Execute a task with Scion.  Delegates to ExecutionEngine."""
        if not self._initialized or not self._execution_engine:
            raise RuntimeError(
                "Scion not initialized. Call await tool_layer.initialize() first or use async with."
            )
    ...
    async def cleanup(self) -> None:
        ...
        """
        logger.info("Cleaning up Scion resources...")
        ...
        try:
            ...
            logger.info("Scion cleanup complete")
    ...
    def get_config(self) -> ScionConfig:
        return self.config

    def list_backends(self) -> List[str]:
        if not self._initialized:
            raise RuntimeError("Scion not initialized")
    ...
    def __repr__(self) -> str:
        ...
        return f"<Scion(status={status}, backends={backends}, model={self.config.llm_model})>"
