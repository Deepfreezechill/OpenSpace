...
from .action_recorder import analyze_agent_actions, format_agent_actions, load_agent_actions
from .utils import generate_summary_report, load_recording_session
...
class RecordingViewer:
    ...
    def show_timeline(self, max_events: int = 50) -> str:
        ...
        for i, item in enumerate(timeline[:max_events]):
            ...
            type_marker = {"agent_action": "🤖", "tool_execution": "🔧"}.get(item.get("type"), "•")
    ...
    def show_agent_flow(self, agent_name: Optional[str] = None) -> str:
        ...
        for action in actions:
            timestamp = (
                action.get("timestamp", "N/A").split("T")[1][:8] if "T" in action.get("timestamp", "") else "N/A"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python -m scion.recording.viewer <recording_dir>")
        sys.exit(1)
