...
Extracted from ``Scion`` (tool_layer.py) in Epic 4.3.  Owns:
...
class ExecutionEngine:
    ...
    ----------
    config:
        The ``ScionConfig`` for iteration budgets, workspace, etc.
    grounding_agent:
    ...
    async def execute(
        ...
        if self._running:
            logger.info(
                "Scion is busy — waiting up to %ds for the current task to finish...",
                _TASK_WAIT_TIMEOUT,
            )
            try:
                ...
                raise RuntimeError(
                    f"Scion is still running after waiting {_TASK_WAIT_TIMEOUT}s. "
        ...
        try:
            ...
            try:
                from scion.grounding.core.types import BackendType as _BT
    ...
    def _resolve_workspace(
        ...
        if workspace_dir:
            ...
            workspace = (
                Path(tempfile.gettempdir()) / "scion_workspace" / task_id
            )
