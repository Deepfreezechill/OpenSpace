...
from scion.agents.base import BaseAgent
from scion.grounding.core.types import BackendType, ToolResult
...
class GroundingAgent(BaseAgent):
        ...
        try:
            while current_iteration < max_iterations:
                ...
                if current_iteration == 2 and self._skill_context:
                    ...
                    messages = [
                        m for m in messages if not (m.get("role") == "system" and m.get("content") == skill_ctx)
                    ]
                ...
                if current_iteration >= 5:
                    messages = self._truncate_messages(messages, keep_recent=8, max_tokens_estimate=120000)
                ...
                if len(assistant_content) > 0:
                    logger.info(
                        f"Iteration {current_iteration} - Assistant content preview: {repr(assistant_content[:300])}"
                    )
                    ...
                    if not has_tool_calls:
                        ...
                        if consecutive_empty_responses >= MAX_CONSECUTIVE_EMPTY:
                            logger.error(
                                f"Exiting due to {MAX_CONSECUTIVE_EMPTY} consecutive empty LLM responses. "
                ...
                if is_complete:
                    ...
                    logger.info(
                        f"Task completed at iteration {current_iteration} (found {GroundingAgentPrompts.TASK_COMPLETE})"
                    )
                    break
                    ...
                    if tool_results_this_iteration:
                        ...
                        logger.debug("Task in progress, LLM did not generate <COMPLETE>")
            ...
            result = {"error": str(e), "status": "error", "instruction": instruction, "iteration": current_iteration}
            self.increment_step()
            return result
    ...
    def construct_messages(self, context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if workspace_dir:
            messages.append({"role": "system", "content": GroundingAgentPrompts.workspace_directory(workspace_dir)})
    ...
    async def _get_available_tools(self, task_description: Optional[str]) -> List:
        ...
        try:
            ...
            logger.info(
                ...
                f"from {len(backends)} backends" + (" [skill-augmented]" if self.has_skill_context else "")
            )
        except Exception as e:
        ...
        if self._skill_registry and self._skill_registry.list_skills():
            from scion.skill_engine.retrieve_tool import RetrieveSkillTool
    ...
    async def _enhance_result_with_visual_context(self, result: ToolResult, tool_name: str) -> ToolResult:
        ...
        try:
            ...
            visual_model = self._visual_analysis_model or (
                self._llm_client.model if self._llm_client else "openrouter/anthropic/claude-sonnet-4.5"
            )
    ...
    def _select_key_screenshots(self, screenshots: List[bytes], max_count: int = 3) -> List[bytes]:
        """
        Select key screenshots if there are too many.
        """
    ...
    def _scan_workspace_files(
        ...
        result = {"files": [], "file_details": {}, "recent_files": []}
