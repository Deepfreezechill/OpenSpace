...
from scion.grounding.core.types import BackendType, SecurityPolicy, SessionConfig
...
class ShellConfig(BackendConfig):
    ...
    mode: Literal["local", "server"] = Field(
        "local", description="Execution mode: 'local' (in-process subprocess) or 'server' (HTTP local_server)"
    )
...
class MCPConfig(BackendConfig):
    ...
    eager_sessions: bool = Field(
        False, description="Whether to eagerly create sessions for all servers on initialization"
    )
    ...
    servers: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="MCP servers configuration, loaded from config_mcp.json"
    )
    sse_read_timeout: float = Field(
        300.0, ge=1.0, le=3600.0, description="SSE read timeout in seconds for HTTP/Sandbox connectors"
    )
...
class GUIConfig(BackendConfig):
    ...
    mode: Literal["local", "server"] = Field(
        "local", description="Execution mode: 'local' (in-process) or 'server' (HTTP local_server)"
    )
...
class ToolSearchConfig(BaseModel):
    ...
    embedding_model: str = Field("BAAI/bge-small-en-v1.5", description="Embedding model name for semantic search")
    max_tools: int = Field(20, ge=1, le=1000, description="Maximum number of tools to return from search")
    search_mode: str = Field("hybrid", description="Default search mode: semantic, keyword, or hybrid")
    enable_llm_filter: bool = Field(True, description="Whether to use LLM for backend/server filtering")
    llm_filter_threshold: int = Field(
        50, ge=1, le=1000, description="Only apply LLM filter when tool count exceeds this threshold"
    )
    enable_cache_persistence: bool = Field(False, description="Whether to persist embeddings to disk")
    cache_dir: Optional[str] = Field(
        None,
        description="Directory for embedding cache. None means use default <project_root>/.scion/embedding_cache",
    )
...
class ToolQualityConfig(BaseModel):
    ...
    enabled: bool = Field(True, description="Whether to enable tool quality tracking")
    enable_persistence: bool = Field(True, description="Whether to persist quality data to disk")
    cache_dir: Optional[str] = Field(
        None, description="Directory for quality cache. None means use default <project_root>/.scion/tool_quality"
    )
    auto_evaluate_descriptions: bool = Field(
        True, description="Whether to automatically evaluate tool descriptions using LLM"
    )
    enable_quality_ranking: bool = Field(True, description="Whether to incorporate quality scores in tool ranking")
    evolve_interval: int = Field(5, ge=1, le=100, description="Trigger quality evolution every N tool executions")
...
class SkillConfig(BaseModel):
    ...
    Built-in skills (``scion/skills/``) are always auto-discovered.
    """

    enabled: bool = Field(True, description="Enable skill matching and injection")
    skill_dirs: List[str] = Field(
        default_factory=list, description="Extra skill directories. Built-in scion/skills/ is always included."
    )
    max_select: int = Field(2, ge=1, le=20, description="Maximum number of skills to inject per task")
...
class GroundingConfig(BaseModel):
    ...
    enabled_backends: List[Dict[str, str]] = Field(
        default_factory=list, description="List of enabled backends, each item: {'name': str, 'provider_cls': str}"
    )

    session_defaults: SessionConfig = Field(
        default_factory=lambda: SessionConfig(
            session_name="", backend_type=BackendType.SHELL, timeout=30, auto_reconnect=True, health_check_interval=30
        )
    )

    tool_cache_ttl: int = Field(300, ge=1, le=3600, description="Tool cache time-to-live in seconds")
    tool_cache_maxsize: int = Field(300, ge=1, le=10000, description="Maximum number of tool cache entries")
