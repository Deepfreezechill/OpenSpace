...
Set via `.env`, MCP config `env` block, or system environment. Scion reads these at startup.

| Variable | Required | Description |
|----------|----------|-------------|
| `SCION_HOST_SKILL_DIRS` | Path A only | Your agent's skill directories (comma-separated). Auto-registered on startup. |
| `SCION_WORKSPACE` | Recommended | Scion project root. Used for recording logs and workspace resolution. |
| `SCION_API_KEY` | No | Cloud API key (`sk-xxx`). Register at https://scion-skills.dev. |
| `SCION_MODEL` | No | LLM model override (default: auto-detected or `openrouter/anthropic/claude-sonnet-4.5`). |
| `SCION_MAX_ITERATIONS` | No | Max agent iterations per task (default: `20`). |
| `SCION_BACKEND_SCOPE` | No | Enabled backends, comma-separated (default: all — `shell,gui,mcp,web,system`). |

### Advanced env overrides (rarely needed)

| Variable | Description |
|----------|-------------|
| `SCION_LLM_API_KEY` | LLM API key (auto-detected from host agent in Path A) |
| `SCION_LLM_API_BASE` | LLM API base URL |
| `SCION_LLM_EXTRA_HEADERS` | Extra HTTP headers for LLM requests (JSON string) |
| `SCION_LLM_CONFIG` | Arbitrary litellm kwargs (JSON string) |
| `SCION_API_BASE` | Cloud API base URL (default `https://scion-skills.dev/api/v1`) |
| `SCION_CONFIG_PATH` | Custom grounding config JSON (deep-merged with defaults) |
| `SCION_SHELL_CONDA_ENV` | Conda environment for shell backend |
| `SCION_SHELL_WORKING_DIR` | Working directory for shell backend |
| `SCION_MCP_SERVERS_JSON` | MCP server definitions (JSON string, merged into `mcpServers`) |
| `SCION_ENABLE_RECORDING` | Record execution traces (default: `true`) |
| `SCION_LOG_LEVEL` | `DEBUG` / `INFO` / `WARNING` / `ERROR` |

## 3. MCP Servers (`config_mcp.json`)

Register external MCP servers that Scion connects to as a **client** (e.g. GitHub, Slack, databases):

```bash
cp scion/config/config_mcp.json.example scion/config/config_mcp.json
```
...
| `config_mcp.json` | MCP servers Scion connects to as a client |
...
| `SCION_ALLOW_UNSANDBOXED` | Environment variable | Set to `1` to allow unsandboxed execution (dev only, **NOT recommended**). |
