...
from scion.local_server.health_checker import HealthChecker
from scion.local_server.platform_adapters import get_platform_adapter
from scion.local_server.utils import AccessibilityHelper, ScreenshotHelper
...
feature_checker = FeatureChecker(platform_adapter=platform_adapter, accessibility_helper=accessibility_helper)
...
"""
            ...
            logger.warning(
                f"Conda environment '{conda_env}' requested but conda not found. Executing with system Python."
            )
            return script
...
def execute_command():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "output": result.stdout, "error": result.stderr, "returncode": result.returncode}
        )
    except subprocess.TimeoutExpired:
        return jsonify({"status": "error", "message": f"Command timeout after {timeout} seconds"}), 408
            ...
            if "window_exists" in verification:
                ...
                try:
                    if platform_name == "Linux":
                        wmctrl_result = subprocess.run(["wmctrl", "-l"], capture_output=True, text=True, check=True)
...
def launch_app():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"{cmd_str} launched successfully"})
    except Exception as e:
...
def run_python():
    ...
    try:
        ...
        if os.path.exists(temp_filename):
            os.remove(temp_filename)
        return jsonify({"status": "error", "message": f"Execution timeout after {timeout} seconds"}), 408
    except Exception as e:
        if os.path.exists(temp_filename):
            os.remove(temp_filename)
        return jsonify({"status": "error", "message": str(e)}), 500
    ...
    try:
        ...
        if os.path.exists(temp_filename):
            try:
                ...
                pass
        return jsonify(
            {"status": "error", "output": f"Failed to execute script: {str(e)}", "error": "", "returncode": -1}
        ), 500
...
def get_desktop_path():
    ...
    try:
        ...
        return jsonify({"status": "success", "path": desktop})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 500
...
def activate_window():
    ...
    try:
        if platform_adapter and hasattr(platform_adapter, "activate_window"):
            ...
            return jsonify({"status": "error", "message": f"Window activation not supported on {platform_name}"}), 501
    except Exception as e:
...
def close_window():
    ...
    try:
        if platform_adapter and hasattr(platform_adapter, "close_window"):
            ...
            return jsonify({"status": "error", "message": f"Window closing not supported on {platform_name}"}), 501
    except Exception as e:
...
def set_wallpaper():
    ...
    try:
        if platform_adapter and hasattr(platform_adapter, "set_wallpaper"):
            ...
            return jsonify({"status": "error", "message": f"Wallpaper setting not supported on {platform_name}"}), 501
    except Exception as e:
...
def start_recording():
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            return jsonify({"status": "error", "message": result.get("message", "Failed to start recording")}), 500
...
def get_terminal_output():
    ...
    try:
        if platform_adapter and hasattr(platform_adapter, "get_terminal_output"):
            ...
            return jsonify({"status": "error", "message": f"Terminal output not supported on {platform_name}"}), 501
    except Exception as e:
...
def upload_file():
    ...
    try:
        ...
        return jsonify({"status": "success", "path": file_path, "message": "File uploaded successfully"})
    except Exception as e:
...
def open_file():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": f"File opened: {path}"})
    except Exception as e:
...
def print_banner(host: str = "127.0.0.1", port: int = 5000, debug: bool = False):
    ...
    from scion.utils.display import colorize, print_section, print_separator
    from scion.utils.display import print_banner as display_banner

    # STARTUP INFORMATION
    display_banner("Scion · Local Server")
    ...
    if host == "0.0.0.0":
        info_lines.append(
            f"{colorize('Listening on all interfaces', 'gr')} {colorize('(0.0.0.0:' + str(port) + ')', 'y')}"
        )
    info_lines.append(
        f"{colorize(platform_name, 'gr')} · {colorize('Debug' if debug else 'Production', 'y' if debug else 'g')}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Scion Local Server - Desktop Control Server")
