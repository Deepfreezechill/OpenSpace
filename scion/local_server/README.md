...
|---|---|---|
| **Setup** | Zero — just run Scion | Start `local_server` first |
...
Set `"mode": "server"` in `scion/config/config_grounding.json`:
...
```bash
# Python entry point
python -m scion.local_server.main --host 127.0.0.1 --port 5000

# Or via helper script
./scion/local_server/run.sh
```
...
Runtime options in `scion/local_server/config.json`:
