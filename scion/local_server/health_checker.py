...
from scion.utils.display import colorize as _c


class HealthStatus:
    """Health status"""

    def __init__(self, feature_available: bool, endpoint_available: Optional[bool], endpoint_detail: str = ""):
...
class HealthChecker:
    ...
    def check_shell_command(self) -> Tuple[bool, str]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/execute", json={"command": "echo hello_test", "shell": True}, timeout=5
            )
    ...
    def check_python_execution(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}/run_python", json={"code": test_code}, timeout=5)
    ...
    def check_bash_script(self) -> Tuple[bool, str]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/run_bash_script", json={"script": "echo bash_test_456"}, timeout=5
            )
    ...
    def check_file_operations(self) -> Tuple[bool, str]:
        ...
        try:
            # Test list directory
            response = requests.post(f"{self.base_url}/list_directory", json={"path": "."}, timeout=5)
    ...
    def check_window_management(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/setup/activate_window", json={"window_name": "NonExistentWindow"}, timeout=5
            )
    ...
    def print_results(self, results: Dict[str, HealthStatus] = None, show_endpoint_details: bool = False):
        """Print check results"""
        if results is None:
        ...
        # Summary
        from scion.utils.display import print_separator

        print()
        print_separator()
        print(
            f"  {_c('Summary:', 'c', bold=True)} {_c(str(feature_available) + '/' + str(total), 'g' if feature_available == total else 'y')} features available",
            end="",
        )
        if any(s.endpoint_available is not None for s in results.values()):
            print(
                f", {_c(str(fully_available) + '/' + str(total), 'g' if fully_available == total else 'y')} fully functional"
            )
        else:
            print()
        print_separator()
        ...
        # Legend
        print(
            f"  {_c('Legend:', 'gr')} {_c('●', 'g')} Available  {_c('●', 'y')} Partial/Untested  {_c('●', 'rd')} Unavailable"
        )
