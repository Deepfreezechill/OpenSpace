...
class FeatureChecker:
    ...
    def check_python_available(self, use_cache: bool = True) -> bool:
        ...
        for python_cmd in python_commands:
            try:
                result = subprocess.run([python_cmd, "--version"], capture_output=True, timeout=2, text=True)
    ...
    def check_window_mgmt_available(self, use_cache: bool = True) -> bool:
        ...
        try:
            ...
            available_methods = [method for method in required_methods if hasattr(self.platform_adapter, method)]
            ...
            if available:
                logger.info(
                    f"Window management check: {'available' if available else 'unavailable'} - supported methods: {', '.join(available_methods)}"
                )
            else:
    ...
    def check_recording_available(self, use_cache: bool = True) -> bool:
        ...
        try:
            ...
            available = hasattr(self.platform_adapter, "start_recording") and hasattr(
                self.platform_adapter, "stop_recording"
            )
