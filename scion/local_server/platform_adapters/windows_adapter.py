...
class WindowsAdapter:
            ...
            try:
                if WINDOWS_LIBS_AVAILABLE:
                    ...
                    if cursor:
                        ...
                        pos = (round(pos_win[0] * ratio - hotspot[0]), round(pos_win[1] * ratio - hotspot[1]))
    ...
    def get_accessibility_tree(self, max_depth: int = 10, max_width: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            tree = self._serialize_uia_element(
                desktop, depth=0, max_depth=max_depth, max_width=max_width, visited=set()
            )
    ...
    def _serialize_uia_element(
        self, element, depth: int = 0, max_depth: int = 10, max_width: int = 50, visited: set = None
        ...
        try:
            ...
            try:
                ...
                result["position"] = {"left": rectangle.left, "top": rectangle.top}
                result["size"] = {"width": rectangle.width(), "height": rectangle.height()}
            except:
                pass
            ...
            try:
                ...
                for i, child in enumerate(children[:max_width]):
                    try:
                        child_data = self._serialize_uia_element(child, depth + 1, max_depth, max_width, visited)
                        if child_data:
        ...
        try:
            ...
            for proc in psutil.process_iter(["pid", "name", "exe"]):
                try:
                    ...
                    apps.append({"name": name, "pid": pinfo["pid"], "path": exe or ""})
