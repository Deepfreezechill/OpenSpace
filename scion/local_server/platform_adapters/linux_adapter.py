...
try:
    import pyatspi
    import Xlib
    from pyatspi import STATE_SHOWING, Accessible, StateType
...
class LinuxAdapter:
    ...
    def get_accessibility_tree(self, max_depth: int = 10, max_width: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            tree = self._serialize_atspi_element(desktop, depth=0, max_depth=max_depth, max_width=max_width)

            return {"tree": tree, "platform": "Linux"}
    ...
    def _serialize_atspi_element(
        self, element: Accessible, depth: int = 0, max_depth: int = 10, max_width: int = 50
        ...
        try:
            ...
            try:
                ...
                result["states"] = [
                    StateType._enum_lookup[st].split("_", 1)[1].lower() for st in states if st in StateType._enum_lookup
                ]
            except:
                result["states"] = []
            ...
            try:
                ...
                for i in range(child_count):
                    try:
                        ...
                        child_data = self._serialize_atspi_element(child, depth + 1, max_depth, max_width)
                        if child_data:
        ...
        try:
            ...
            for line in result.stdout.strip().split("\n"):
                if line:
                    ...
                    if len(parts) >= 4:
                        windows.append({"id": parts[0], "desktop": parts[1], "hostname": parts[2], "title": parts[3]})
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"platform": "Linux", "distro": distro, "kernel": kernel, "available": self.available}
    ...
    def start_recording(self, output_path: str) -> Dict[str, Any]:
        try:
            try:
                ...
                return {"status": "error", "message": "ffmpeg not installed. Install with: sudo apt install ffmpeg"}
            ...
            if process.poll() is not None:
                ...
                return {"status": "error", "message": f"Failed to start recording: {error_output}"}

            logger.info(f"Linux recording started: {output_path}")
            return {"status": "success", "message": "Recording started", "process": process}
