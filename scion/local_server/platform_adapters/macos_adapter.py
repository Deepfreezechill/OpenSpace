...
class MacOSAdapter:
    ...
    def get_accessibility_tree(self, max_depth: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"app_name": app_name, "bundle_id": bundle_id, "tree": tree, "platform": "macOS"}

            except Exception as e:
                logger.error(f"Cannot get app reference: {e}")
                return {"error": f"Cannot get app reference: {e}", "app_name": app_name, "bundle_id": bundle_id}
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"platform": "macOS", "version": version, "model": model, "available": self.available}

        except Exception as e:
            logger.error(f"Failed to get system information: {e}")
            return {"platform": "macOS", "error": str(e)}
    ...
    def start_recording(self, output_path: str) -> Dict[str, Any]:
        try:
            ...
            command = [
                ...
                "30",
                "-i",
                screen_dev,  # Use detected screen device
            ]
            ...
            process = subprocess.Popen(
                command, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True
            )
            ...
            if process.poll() is not None:
                ...
                return {"status": "error", "message": f"Failed to start recording: {error_output[:300]}"}
            ...
            if result.returncode == 0 and result.stdout:
                ...
                if output:
                    ...
                    for i in range(0, len(items), 2):
                        if i + 1 < len(items):
                            windows.append({"app_name": items[i], "window_title": items[i + 1]})
    ...
    def get_terminal_output(self) -> Optional[str]:
        ...
        try:
            ...
            result = subprocess.run(["osascript", "-e", iterm_script], capture_output=True, text=True, timeout=5)
