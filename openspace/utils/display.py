...
BOX_CHARS = {
    BoxStyle.ROUNDED: {"tl": "╭", "tr": "╮", "bl": "╰", "br": "╯", "h": "─", "v": "│"},
    BoxStyle.SQUARE: {"tl": "┌", "tr": "┐", "bl": "└", "br": "┘", "h": "─", "v": "│"},
    BoxStyle.DOUBLE: {"tl": "╔", "tr": "╗", "bl": "╚", "br": "╝", "h": "═", "v": "║"},
}
...
class Box:
    def __init__(self, width: int = 68, style: BoxStyle = BoxStyle.ROUNDED, color: str = "bl", padding: int = 2):
        self.width = width
        self.style = style
        self.color = color
    ...
    def top_line(self, indent: int = 2) -> str:
        ...
        return colorize(indent_str + self.chars["tl"] + self.chars["h"] * self.width + self.chars["tr"], self.color)

    def bottom_line(self, indent: int = 2) -> str:
        indent_str = " " * indent
        if self.style == BoxStyle.SIMPLE:
            return colorize(indent_str + "=" * self.width, self.color)
        return colorize(indent_str + self.chars["bl"] + self.chars["h"] * self.width + self.chars["br"], self.color)
    ...
    def empty_line(self, indent: int = 2) -> str:
        ...
        if self.style == BoxStyle.SIMPLE:
            return ""
        return colorize(indent_str + self.chars["v"] + " " * self.width + self.chars["v"], self.color)
    ...
    def text_line(self, text: str, align: str = "left", indent: int = 2, text_color: str = "") -> str:
        ...
        return (
            colorize(indent_str + self.chars["v"], self.color)
            + padding_str
            + content
            + padding_str
    ...
    def build(
        self, title: Optional[str] = None, lines: List[str] = None, footer: Optional[str] = None, indent: int = 2
    ) -> str:
        result = []
...
def print_section(title: str, content: List[str], color: str = "c", indent: int = 2):
    indent_str = " " * indent
