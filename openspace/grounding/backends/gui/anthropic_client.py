...
class AnthropicGUIClient:
    ...
    async def plan_action(
        ...
        if self.messages and self.messages[-1]["role"] == "assistant":
            ...
            if isinstance(last_content, list) and any(block.get("type") == "tool_use" for block in last_content):
                tool_use_id = next(block["id"] for block in last_content if block.get("type") == "tool_use")
        ...
        system = BetaTextBlockParam(type="text", text=get_system_prompt(self.platform))
    ...
    def _add_tool_result(self, tool_use_id: str, result: str, screenshot_bytes: Optional[bytes] = None):
        """
        ...
        tool_result_content = [{"type": "tool_result", "tool_use_id": tool_use_id, "content": content_list}]
