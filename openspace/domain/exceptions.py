...
All domain-specific exceptions inherit from :class:`OpenSpaceError`.
Each subclass maps to a stable ``error_code`` that the MCP layer can
surface to callers without leaking internals.

Usage::

    from openspace.domain.exceptions import ValidationError, NotFoundError
...
class OpenSpaceError(Exception):
...
class ValidationError(OpenSpaceError):
    """Bad input from the caller (missing / invalid args)."""

    error_code = "VALIDATION_ERROR"


class NotFoundError(OpenSpaceError):
...
class PermissionDeniedError(OpenSpaceError):
    """Authentication or authorization failure."""

    error_code = "PERMISSION_DENIED"


class OperationTimeoutError(OpenSpaceError):
...
class ExecutionError(OpenSpaceError):
    """Task execution failed at runtime."""

    error_code = "EXECUTION_ERROR"


class DependencyError(OpenSpaceError):
...
class ConfigurationError(OpenSpaceError):
    """Invalid or missing configuration."""

    error_code = "VALIDATION_ERROR"


class ExternalServiceError(OpenSpaceError):
...
class SandboxError(OpenSpaceError):
    """Sandbox creation, execution, or teardown failure."""

    error_code = "EXECUTION_ERROR"


class EvolutionError(OpenSpaceError):
    """Skill evolution failed."""

    error_code = "EXECUTION_ERROR"


class InternalError(OpenSpaceError):
