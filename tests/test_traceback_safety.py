...
def _assert_no_traceback_leak(response_str: str, tool_name: str) -> None:
    ...
    for pat in _FORBIDDEN_PATTERNS:
        assert not pat.search(response_str), (
            f"[{tool_name}] Traceback leak detected — pattern {pat.pattern!r} "
            f"found in response:\n{response_str[:500]}"
        )
...
def _assert_structured_error(response_str: str, tool_name: str) -> dict:
    ...
    assert data.get("isError") is True, (
        f"[{tool_name}] Expected isError=true, got {data}"
    )
    assert data.get("error_code") in VALID_ERROR_CODES, (
        f"[{tool_name}] Invalid error_code: {data.get('error_code')}"
    )
    assert isinstance(data.get("message"), str) and data["message"], (
        f"[{tool_name}] Missing or empty 'message'"
    )
...
class TestSafeErrorResponse:
    ...
    def test_schema(self):
        from openspace.errors import safe_error_response, EXECUTION_ERROR
    ...
    def test_custom_correlation_id(self):
        from openspace.errors import safe_error_response, VALIDATION_ERROR
...
class TestHandleMcpException:
    ...
    def test_logs_and_returns_safe_json(self):
        ...
        with patch("openspace.errors.logger") as mock_logger:
            ...
            result = handle_mcp_exception(
                exc, tool_name="test_tool", error_code=EXECUTION_ERROR
            )
    ...
    def test_correlation_id_in_log_and_response(self):
        from openspace.errors import handle_mcp_exception, INTERNAL_ERROR

        with patch("openspace.errors.logger") as mock_logger:
            exc = ValueError("oops")
            result = handle_mcp_exception(
                exc, tool_name="x", error_code=INTERNAL_ERROR
            )
            ...
            assert cid in str(log_args), (
                f"Correlation ID {cid} not found in log call: {log_args}"
            )
...
@pytest.mark.parametrize("tool_name,call", [
    ...
    ("fix_skill", lambda srv: srv.fix_skill(
        skill_dir="/nonexistent", direction="fix")),
...
async def test_all_tools_structured_error_on_failure(tool_name, call):
    ...
    assert data.get("error_code") in VALID_ERROR_CODES, (
        f"[{tool_name}] Bad error_code: {data.get('error_code')}"
    )
