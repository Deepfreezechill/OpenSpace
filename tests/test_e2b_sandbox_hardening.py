...
class TestSandboxEnforcement:
    ...
    def test_mcp_config_defaults_sandbox_true(self):
        ...
        from scion.config.grounding import MCPConfig
    ...
    def test_config_grounding_json_sandbox_true(self):
        ...
        config_path = ROOT / "scion" / "config" / "config_grounding.json"
    ...
    def test_config_security_json_sandbox_enabled(self):
        ...
        config_path = ROOT / "scion" / "config" / "config_security.json"
        data = json.loads(config_path.read_text())
    ...
    def test_mcp_client_constructor_defaults_sandbox_true(self):
        ...
        from scion.grounding.backends.mcp.client import MCPClient
    ...
    def test_create_connector_defaults_sandbox_true(self):
        ...
        from scion.grounding.backends.mcp.config import create_connector_from_config
    ...
    def test_provider_extracts_sandbox_default_true(self):
        ...
        source = (ROOT / "scion" / "grounding" / "backends" / "mcp" / "provider.py").read_text()
...
class TestConfigSourceRestriction:
    ...
    def test_e2b_sandbox_ignores_caller_api_key(self):
        ...
        source = (ROOT / "scion" / "grounding" / "core" / "security" / "e2b_sandbox.py").read_text()
    ...
    def test_trusted_sandbox_options_strips_api_key(self):
        ...
        from scion.grounding.backends.mcp.config import _build_trusted_sandbox_options
...
class TestFailClosedBehavior:
    ...
    async def test_unsandboxed_stdio_denied_by_default(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            # Ensure SCION_ALLOW_UNSANDBOXED is not set
            os.environ.pop("SCION_ALLOW_UNSANDBOXED", None)
    ...
    async def test_unsandboxed_stdio_allowed_with_explicit_opt_out(self):
        """Stdio without sandbox works only with SCION_ALLOW_UNSANDBOXED=1."""
        from scion.grounding.backends.mcp.config import create_connector_from_config
        from scion.grounding.backends.mcp.transport.connectors import StdioConnector

        config = {"command": "python", "args": ["-m", "some_server"]}
        with patch.dict(os.environ, {"SCION_ALLOW_UNSANDBOXED": "1"}):
    ...
    async def test_unsandboxed_partial_value_still_denied(self):
        """SCION_ALLOW_UNSANDBOXED must be exactly '1'."""
        from scion.grounding.backends.mcp.config import create_connector_from_config

        config = {"command": "python", "args": ["-m", "some_server"]}
        for bad_value in ["true", "yes", "0", "", "  "]:
            with patch.dict(os.environ, {"SCION_ALLOW_UNSANDBOXED": bad_value}):
                with pytest.raises(RuntimeError, match="Unsandboxed stdio execution denied"):
                    await create_connector_from_config(
                        config,
                        server_name="test",
    ...
    async def test_sandbox_required_but_e2b_unavailable_raises(self):
        ...
        from scion.grounding.backends.mcp import config as cfg_module
        from scion.grounding.backends.mcp.config import create_connector_from_config as _create
    ...
    def test_config_loader_fails_closed_on_invalid_config(self):
        ...
        source = (ROOT / "scion" / "config" / "loader.py").read_text()
    ...
    async def test_sandbox_enforcement_before_ensure_dependencies(self):
        ...
        source = (ROOT / "scion" / "grounding" / "backends" / "mcp" / "config.py").read_text()
    ...
    async def test_unsandboxed_denied_before_deps_installed(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SCION_ALLOW_UNSANDBOXED", None)
            with patch(
                "scion.grounding.backends.mcp.installer.MCPInstallerManager.ensure_dependencies",
...
class TestSandboxDocumentation:
    ...
    def test_env_example_documents_e2b_api_key_required(self):
        ...
        env_example = (ROOT / "scion" / ".env.example").read_text()
    ...
    def test_env_example_documents_unsandboxed_opt_out(self):
        """SCION_ALLOW_UNSANDBOXED is documented."""
        env_example = (ROOT / "scion" / ".env.example").read_text()
        assert "SCION_ALLOW_UNSANDBOXED" in env_example

    def test_config_readme_documents_sandbox(self):
        """config/README.md has E2B sandbox section."""
        readme = (ROOT / "scion" / "config" / "README.md").read_text()
        ...
        assert "SCION_ALLOW_UNSANDBOXED" in readme, "README must document SCION_ALLOW_UNSANDBOXED"
...
class TestSandboxCreationIntegration:
    ...
    async def test_stdio_config_creates_sandbox_connector(self):
        ...
        from scion.grounding.backends.mcp.config import create_connector_from_config
        from scion.grounding.backends.mcp.transport.connectors import SandboxConnector
        ...
        with (
            patch(
                "scion.grounding.backends.mcp.config.E2BSandbox",
                return_value=mock_e2b,
            ),
            patch("scion.grounding.backends.mcp.config.E2B_AVAILABLE", True),
        ):
            connector = await create_connector_from_config(
                config,
                server_name="test",
    ...
    async def test_http_config_unaffected_by_sandbox_enforcement(self):
        ...
        from scion.grounding.backends.mcp.config import create_connector_from_config
        from scion.grounding.backends.mcp.transport.connectors import HttpConnector
    ...
    async def test_websocket_config_unaffected_by_sandbox_enforcement(self):
        ...
        from scion.grounding.backends.mcp.config import create_connector_from_config
        from scion.grounding.backends.mcp.transport.connectors import WebSocketConnector
...
class TestNoSandboxFalseInProduction:
    ...
    PRODUCTION_DIRS = [
        ROOT / "scion" / "grounding" / "backends" / "mcp",
        ROOT / "scion" / "config",
    ]
