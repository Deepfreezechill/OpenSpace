...
class TestIsAutoImportEnabled:
    ...
    def test_returns_false_when_not_initialized(self):
        ...
        try:
            ...
            srv._openspace_instance = mock_os
            assert srv._is_auto_import_enabled() is False
        finally:
            srv._openspace_instance = original
...
class TestCloudSearchAndImportGating:
    ...
    def _patch_auto_import(self):
        with patch("scion.mcp_server._is_auto_import_enabled", return_value=False):
            yield

    async def test_returns_empty_when_disabled(self):
        from scion.mcp_server import _cloud_search_and_import
...
class TestCloudSearchAndImportEnabled:
    ...
    async def test_proceeds_when_enabled(self):
        ...
        with patch("scion.mcp_server._is_auto_import_enabled", return_value=True):
            from scion.mcp_server import _cloud_search_and_import
...
class TestDoImportCloudSkillGating:
    ...
    async def test_blocked_when_disabled(self):
        with patch("scion.mcp_server._is_auto_import_enabled", return_value=False):
            from scion.mcp_server import _do_import_cloud_skill
...
class TestSearchSkillsAutoImportGating:
    ...
    @pytest.fixture(autouse=True)
    def _patch_openspace(self):
        """Patch _get_openspace to avoid full initialization."""
        mock_os = MagicMock()
        mock_os.is_initialized.return_value = True
        ...
        with (
            ...
            patch("scion.mcp_server._get_store") as mock_store,
            patch("scion.cloud.search.hybrid_search_skills", mock_hybrid, create=True),
            patch("scion.mcp_server._is_auto_import_enabled", return_value=False),
            patch("scion.mcp_server._do_import_cloud_skill", new_callable=AsyncMock) as mock_import,
        ):
...
class TestExecuteTaskCloudImportGating:
    ...
    async def test_cloud_import_blocked_in_execute_task(self):
        ...
        with (
            patch("scion.mcp_server._is_auto_import_enabled", return_value=False),
            patch("scion.mcp_server._cloud_search_and_import", new_callable=AsyncMock) as mock_cloud,
        ):
