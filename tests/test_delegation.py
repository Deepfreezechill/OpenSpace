"""Tests for EPIC 1.4 — OpenSpace Delegation.
...
Validates:
- Legacy creation path (OpenSpaceConfig only) still works
...
class TestLegacyCreation:
    ...
    def test_default_config(self):
        """OpenSpace() with no args still works."""
        cs = OpenSpace()
...
class TestContainerCreation:
    ...
    def test_from_container(self):
        """from_container() classmethod creates OpenSpace with container."""
        container = AppContainer(llm=_StubLLM())
        cs = OpenSpace.from_container(container)
    ...
    def test_init_with_container_kwarg(self):
        ...
        cs = OpenSpace(container=container)
        assert cs.container is container
    ...
    def test_container_default_config(self):
        ...
        cs = OpenSpace.from_container(AppContainer())
        assert cs.config is not None
...
class TestRegression:
    ...
    def test_not_initialized_by_default(self):
        ...
        legacy = OpenSpace()
        container_based = OpenSpace.from_container(AppContainer())
    ...
    def test_config_accessible(self):
        ...
        config = OpenSpaceConfig(llm_model="test/model")
        legacy = OpenSpace(config=config)
        container_based = OpenSpace.from_container(AppContainer(), config=config)
