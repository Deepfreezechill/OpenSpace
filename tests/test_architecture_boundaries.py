...
_SCION = _REPO_ROOT / "scion"
_DOMAIN = _SCION / "domain"
...
class TestDomainImportPurity:
    """Domain layer (scion/domain/) must import ZERO infrastructure modules.
    ...
    def test_domain_imports_no_infrastructure(self) -> None:
        """Every import in scion/domain/ must be stdlib, structlog, or domain-local."""
        violations: list[str] = []
    ...
    def test_domain_does_not_import_scion_infra(self) -> None:
...
# Private fields that belong to Scion internals
...
_MCP_HANDLER_FILES: list[Path] = [
    _SCION / "mcp_server.py",
]


class TestMCPHandlerBoundary:
    """MCP handlers must access Scion through public API only.
    ...
    def test_no_private_field_access(self) -> None:
        """MCP handlers must not access private fields of Scion."""
        violations: list[str] = []
...
class TestFileSizeGuard:
    ...
    def _all_source_files() -> list[Path]:
        """Collect all .py files under scion/."""
        return _collect_py_files(_SCION)
